...
from transformers import RobertaTokenizer, EncoderDecoderModel, Trainer, TrainingArguments, GPT2LMHeadModel
import torch
...
model_name = 'roberta-base'  # Mudança para o modelo roberta-base
tokenizer = RobertaTokenizer.from_pretrained(model_name)

gpt2 = GPT2LMHeadModel.from_pretrained("gpt2")

model = EncoderDecoderModel.from_encoder_decoder_pretrained(model_name, 'gpt2')

# Definir
model.config.decoder_start_token_id = tokenizer.bos_token_id  # Alterado para BOS token do RoBERTa
model.config.pad_token_id = tokenizer.pad_token_id
model.config.eos_token_id = tokenizer.eos_token_id  # Alterado para EOS token do RoBERTa
...
training_args = TrainingArguments(
    ...
    learning_rate=3e-5,
    per_device_train_batch_size=2,  # Reduzir o batch size
    per_device_eval_batch_size=2,   # Reduzir o batch size
    num_train_epochs=1,
    weight_decay=0.01,
    logging_dir='./logs',
    logging_steps=1000,
...
trainer.save_model("./models-tuned/roberta/model")
tokenizer.save_pretrained("./models-tuned/roberta/tokenizer")
